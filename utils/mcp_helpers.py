...
# JSON schema "type" → Python isinstance check
...
def _matches_type(value: Any, expected_type: Optional[str]) -> bool:
    """JSON schema 类型粗校验。expected_type 为 None / 未知 → 不校验返回 True。

    注意：bool 在 Python 是 int 子类，所以先单独判 bool。
    """
    if expected_type is None or expected_type not in _TYPE_MAP:
        return True
    if expected_type == "integer" and isinstance(value, bool):
        return False  # bool 不算 integer (避免 True 通过 int 校验)
    if expected_type == "number" and isinstance(value, bool):
        return False
    return isinstance(value, _TYPE_MAP[expected_type])
...
class EduMCPServer:
    ...
    def call_tool(self, name: str, arguments: dict, user_id: Optional[str] = None) -> Any:
        ...
        # Schema validation: required + 类型粗校验（防 LLM 传 "5" 给 int 参数后下游 TypeError）
        ...
        for arg_name, arg_value in arguments.items():
            if arg_name not in properties:
                continue  # extra args 容忍（不强制 strict）
            expected_type = properties[arg_name].get("type")
            if not _matches_type(arg_value, expected_type):
                raise TypeError(
                    f"Tool '{name}' arg '{arg_name}' expected {expected_type}, "
